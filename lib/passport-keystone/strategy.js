...
util = require('util'),
Identity = require('pkgcloud_v.1.3_hack/lib/pkgcloud/openstack/identity');
 ...
 *   - 'domainNameField' field name where the domainName is found, defaults to _domainName_
 *   - 'region'         sets region
...
function Strategy(options, verify) {
  ...
  this._domainNameField = options.domainNameField || 'domainName';
      ...
      if (err) {
          ...
          newClient.auth(function(err){
              if (err) {
                  ...
                  if (!newClient._identity.token.id) return self.fail('Auth token not generated');
                  if (self._verify) {
                      if (self._passReqToCallback) {
                          return self._verify(req, newClient._identity, verified(self));
                      } else {
                          return self._verify(newClient._identity, verified(self));
                      }
                  } else {
                      return self.success(newClient._identity);
                  }
              }
          });
      } else {
